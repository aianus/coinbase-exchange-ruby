...
describe LiveOrderbook, vcr: vcr_options do
  ...
  let(:one_arg_callback) { lambda { |_| nil } }
  let(:zero_arg_callback) { lambda {} }

  it "queues up websocket messages from before the orderbook is loaded" do
    orderbook.on_ready(&zero_arg_callback)
    orderbook.on_open(&one_arg_callback)
    ...
    expect(zero_arg_callback).to receive(:call).once
    expect(one_arg_callback).to receive(:call).once
